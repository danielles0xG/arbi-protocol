...
contract KyberswapV2 is OwnableUpgradeable, IExchange {
    ...
    function initialize(address router_, address _factoryV2)
        external
        initializer
    {
        __Ownable_init();
    ...
    function _wrapIERC20(address[] calldata _addresses, uint160 _limit)
        internal
    ...
    function getReserves(address poolAddress_)
        external
        view
        returns (
    ...
    function getTradeInfo(address _poolAddress)
        public
        view
        returns (
            uint112 _vReserve0,
    ...
    {
        (_vReserve0, _vReserve1, reserve0, reserve1, feeInPrecision) = IDMMPool(
            _poolAddress
        ).getTradeInfo();
    }
